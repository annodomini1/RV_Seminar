...
class Camera(object):
    ...
    """
    def __init__(self, name = 'neither left nor right', device_number=0, exposure=None, w=1280, h=1024):
        ...
        self.overlay_text = ['ESC: close window'] # a list of strings, must be a list even if has only one
        ...
        # calibration pattern properties (points where black squares' egdes meet)
        self.rows = 7           # n-1
        self.columns = 9        # m-1
        self.square_size = 2.54 # in cm
        ...
        if self.load_intrinsics():
            print("intrinsic calibration parameters for camera " + self.name + ' were loaded succefully \n')
            self.is_calibrated = True

                #Try to load previously saved calibration parameters
        if self.load_extrinsics():
            print("extrinsic calibration parameters for camera " + self.name + ' were loaded succefully \n')
    ...
    def show_controls(self):
        ...
        # NOTE: commented out
        cv2.createTrackbar("Exposure",
        	self.controls_window_name,
        	int(-self.cap.get(cv2.CAP_PROP_EXPOSURE)), 11, 
    ...
    def calibrate_gui(self):
        ...
        # add text
        self.overlay_text = ['a: acquire one image',
                             'c: try to calibrate',
                             't: toggle camera conrols',
                             'r: remove previous frame',
                             'ESC: exit',
        ...
        #self.calibrator = calibration.StereoCalibrator(rows= 6, columns = 9,square_size = 2.54, image_size = (self.w, self.h))
    ...
    def return_chessboard_corners_image(self):
        temp = self.im.copy()
        cv2.drawChessboardCorners(temp, (self.rows, self.columns), self.calibration_points[-1].reshape(-1,1,2),
                                      True)
        return temp
    ...
    def calibration_append_and_show(self):
        if self.calibration_append():
            ...
            # show
            cv2.drawChessboardCorners(self.im, (self.rows, self.columns), self.calibration_points[-1].reshape(-1,1,2),
                                      True)
        ...
        if not ret:
            ...
            cv2.cornerSubPix(temp, corners, (5, 5), (-1, -1),
                             (cv2.TERM_CRITERIA_MAX_ITER + cv2.TERM_CRITERIA_EPS,
                              30, 0.01))
            # APPEND
            self.calibration_points.append(corners.reshape(-1, 2)) # reshapeth!, adopted from StereoVision module, removes a singleton dim (LIKE MATLAB SQUEEZE)
    ...
    def remove_and_show(self):
        ...
        cv2.imshow(self.chessboard_window_name,self.add_overlay_text(self.im.copy(), text=['Calibration frame removed']))
        # a one-liner to replace the count
        self.overlay_text[5] = ' '.join([str(len(self.calibration_points))] + self.overlay_text[5].split()[1:3])
    ...
    def calibrate(self):
        ...
        retval, cameraMatrix, distCoeffs, rvecs, tvecs = cv2.calibrateCamera(np.array(self.world_points),
                                                                             np.array(self.calibration_points),(self.w, self.h),None,None)#,cameraMatrix,dist_coef)
    ...
    def add_overlay_text(self, im=False, text=False):
        ...
        for i in range(nlines):
            cv2.putText(im, text[i], (0, self.h-(nlines-i-1)*self.overlay_text_line_spacing-self.overlay_text_line_spacing/2),
    ...
    def save_extrinsics(self, filename = None):
        ...
        try:
            with open(filename,'wb') as f:
                pickle.dump(to_save,f)
        except EnvironmentError: #cant open file
